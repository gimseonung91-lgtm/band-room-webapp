...
function renderScores() {
  ...
  const driveError = (state.data.googleErrors || []).find(
    (message) => message.includes('Drive') || message.includes('드라이브')
  );
  if (state.scoreError) {
    $('#fileList').innerHTML = `<div class="empty-state error-text">${escapeHtml(state.scoreError)}</div>`;
    return;
  }
  if (driveError) {
    $('#fileList').innerHTML = `<div class="empty-state error-text">${escapeHtml(driveError)}</div>`;
    return;
  }
  if (!state.data.googleReady) {
    $('#fileList').innerHTML = '<div class="empty-state">Google 연결을 완료하면 악보 폴더의 파일을 불러옵니다.</div>';
    return;
  }
  if (!state.data.setup.hasDriveFolder) {
    $('#fileList').innerHTML = '<div class="empty-state">Netlify 환경 변수에 GOOGLE_DRIVE_FOLDER_ID를 넣어주세요.</div>';
    return;
  }
  if (files.length === 0) {
    $('#fileList').innerHTML = '<div class="empty-state">악보 폴더에 파일을 넣으면 목록이 표시됩니다.</div>';
    return;
  }
...
async function refreshScores() {
  try {
    ...
    state.scoreError = error.message;
  }
  renderScores();
}
